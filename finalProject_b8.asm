...
check_char: add $t1, $s0, $t0 		# t1 = address of string[i], dia chi ky tu hien tai trong chuoi
...
test_length: move $t5, $t3
	and 	$t1, $t3, 0x0000000f		# xoa het cac byte cua $t3 ve 0, chi giu lai byte cuoi=4bit cuoi
...
split1:addi 	$t0, $zero, 0			# so byte duoc in ra (4 byte)
	addi 	$t9, $zero, 0 #chi so dem so byte duoc in ra tu o dia 1
	addi 	$t8, $zero, 0 #chi so dem so byte duoc in ra tu o dia 3
	la 	$s1, d1
	la 	$s2, d2
	la 	$a2, array
print11:li 	$v0, 4
	la 	$a0, m3
	syscall
b11:	lb 	$t1, ($s0)  #doc byte tu o dia 1 tai dia chi s0
	addi 	$t3, $t3, -1 #giam gia tri cua t3 (so byte con lai can in)
	sb 	$t1, ($s1) #ghi byte cua o dia 1 tai dia chi s1
b21:	add 	$s5, $s0, 4 #tinh dia chi cua byte tiep theo trong o dia 2
	lb 	$t2, ($s5)			# doc byte tu o dia 2 tai dia chi s5
	addi 	$t3, $t3, -1 #giam gia tri cua t3
	sb 	$t2, ($s2) #ghi byte cua o dia 2 tai dia chi s2
b31:	xor 	$a3, $t1, $t2 #tinh toan parity
	sw 	$a3, ($a2) #luu ket qua parity
	addi 	$a2, $a2, 4 #di chuyen con tro mang parity len 4 byte
	addi 	$t0, $t0, 1 #tang gia tri cua t0 (dem so byte da xu ly)
	addi 	$s0, $s0, 1
	addi 	$s1, $s1, 1
	addi 	$s2, $s2, 1
	bgt 	$t0, 3, reset
...
reset:	la 	$s1, d1
	la 	$s2, d2
print12:lb 	$a0, ($s1)  #doc byte tu o dia 1 tai dia chi s1
	li 	$v0, 11
	syscall
	addi 	$t9, $t9, 1 
	addi 	$s1, $s1, 1
	bgt 	$t9, 3, next11
	j 	print12	#quay lai in byte tiep tu o dia 1
next11:li 	$v0, 4
	la 	$a0, m4
	syscall
	li 	$v0, 4
...
next12:li 	$v0, 4
	...
	la 	$a2, array #dat lai con tro mang array
	addi 	$t9, $zero, 0 #khoi tao t9 lai ve 0
print14:lb 	$t8, ($a2) #doc byte tu mang array tai dua chi a2
	jal 	HEX #chuyen byte thanh hex
	li 	$v0, 4
	la 	$a0, comma
	syscall
	addi 	$t9, $t9, 1
